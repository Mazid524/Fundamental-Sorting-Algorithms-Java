...
//        else n=5500000;
        ...
        System.out.println("End Start "+  startTime);
...
//        System.out.println("The algorithm took "+tookTime+" nano seconds");
    ...
    public static void insertionSort(SDATA[] A, int n){
        int i;
        int j;
        SDATA key;
        for(j=1; j<n; j++){
            key = A[j];
            i=j-1;
            while((i>=0) && (A[i].info >= key.info)){
                if(A[i].info == key.info){
                    if(A[i].infof >= key.infof){
                        A[i+1] = A[i];
                    }
                }
                else{
